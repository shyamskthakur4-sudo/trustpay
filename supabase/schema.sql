...
create table if not exists public.deposits (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.wallet_accounts(id), network text not null,
  amount_usdt numeric(30,6) not null check (amount_usdt >= 500), tx_hash text not null,
  status text not null default 'pending' check (status in ('pending','verified','rejected','needs_review')),
  confirmations integer not null default 0, verified_amount_usdt numeric(30,6), verified_at timestamptz,
  verified_by uuid references auth.users(id), created_at timestamptz not null default now(), unique(network,tx_hash)
);

create table if not exists public.withdrawals (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.wallet_accounts(id),
  amount_usdt numeric(30,6) not null check (amount_usdt >= 500), rate_inr numeric(18,2) not null default 107,
  amount_inr numeric(30,2) generated always as (amount_usdt * rate_inr) stored,
  method text not null check (method in ('UPI','IMPS')), payout_details_encrypted text not null,
  status text not null default 'pending' check (status in ('pending','approved','paid','rejected')),
  created_at timestamptz not null default now(), approved_at timestamptz, paid_at timestamptz, approved_by uuid references auth.users(id)
);
...
create policy "users read own admin membership" on public.admin_users for select to authenticated using (user_id=auth.uid());
